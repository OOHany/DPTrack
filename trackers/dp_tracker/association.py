...
def get_deep_range_low(obj, step=1):
    col = []
    for t in obj:
        lend = t[-1]
        col.append(lend)
...
def tb_dist(bboxes1, bboxes2):
    ...
    # tb_dist = np.abs(bboxes1[..., 1] - bboxes2[..., 1]) + np.abs(bboxes1[..., 3] - bboxes2[..., 3]) + np.abs(bboxes1[..., 0] - bboxes2[..., 0]) + np.abs(bboxes1[..., 2] - bboxes2[..., 2])
    # tb_dist = np.abs(bboxes1[..., 0] - bboxes2[..., 0]) + np.abs(bboxes1[..., 2] - bboxes2[..., 2])
...
def Biou(bboxes1, bboxes2, level=1):
    ...
    yy11_W = np.maximum(bboxes1[..., 0], bboxes2[..., 0])  # 交集左上角点x坐标
    yy12_W = np.minimum(bboxes1[..., 2], bboxes2[..., 2])  # 交集右下角点x坐标

    yy21_W = np.minimum(bboxes1[..., 0], bboxes2[..., 0])  # 并集左上角点x坐标
    yy22_W = np.maximum(bboxes1[..., 2], bboxes2[..., 2])  # 并集右下角点x坐标
    '''扩展后的box的交集与并集'''
    yy11_B = np.maximum(bboxes1_B[..., 1], bboxes2_B[..., 1])  # 交集左上角点y坐标
    yy12_B = np.minimum(bboxes1_B[..., 3], bboxes2_B[..., 3])  # 交集右下角点y坐标

    yy21_B = np.minimum(bboxes1_B[..., 1], bboxes2_B[..., 1])  # 并集左上角点y坐标
    yy22_B = np.maximum(bboxes1_B[..., 3], bboxes2_B[..., 3])  # 并集右下角点y坐标

    yy11_WB = np.maximum(bboxes1_B[..., 0], bboxes2_B[..., 0])  # 交集左上角点x坐标
    yy12_WB = np.minimum(bboxes1_B[..., 2], bboxes2_B[..., 2])  # 交集右下角点x坐标

    yy21_WB = np.minimum(bboxes1_B[..., 0], bboxes2_B[..., 0])  # 并集左上角点x坐标
    yy22_WB = np.maximum(bboxes1_B[..., 2], bboxes2_B[..., 2])  # 并集右下角点x坐标
    '''原始box的HIOU'''
    o = (yy12 - yy11) / (yy22 - yy21)  # HIOU
    '''原始box的WIOU'''
